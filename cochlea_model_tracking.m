% Function:
% 1. Select a FOLDER containing several MP4 videos (V01, V02, ...)
% 2. Process every video one after another with the same algorithm:
%    - Read all frames using VideoReader
%    - Median filter each RGB channel (image stays RGB)
%    - Background subtraction -> BW (electrode = foreground)
%    - Track the tip across all frames -> TipCoordinates
%    - Manual review/correction, plot tip trajectory and insertion angle
% 3. Each video gets its own .mat file (TipCoordinates, cochleaCenter, ...)
% 4. Every figure title additionally shows which V0x is being processed
% 5. After each video the program waits for confirmation before the next one;
%    all result plots stay open.

clear; clc; close all;

%% 1. Select FOLDER with MP4 videos
...
% --- Gemeinsame Parameter (für alle Videos identisch) ---
startFrame  = 40;    % Ab welchem Frame getrackt wird (Elektrode anfangs nicht im Bild)
...
tipRadius   = 20;    % Radius der Tip-Suchmaske
curvatureThreshold = 0.06;  % Schwelle für Krümmungssprung -> manuelle Kontrolle
...
for vi = 1:numVideos
    ...
    sf = min(max(round(startFrame), 1), totalFrames);
    ImageNummax = totalFrames - sf + 1;
    ...
    % Initialize structure array (frisch pro Video)
    ...
    for n = 1:ImageNummax
        frameIdx = sf + n - 1;   % echte Frame-Nummer im Video
        img = read(v, frameIdx);
    ...
    useExisting = false;
    if isfile(trajFile)
        choice = questdlg(['[' vLabel '] Für dieses Video existiert bereits eine gespeicherte ', ...
            'Trajektorie. Möchtest du die alte verwenden oder eine neue zuweisen?'], ...
            ['Trajektorie laden (' vLabel ')'], 'Alte verwenden', 'Neu zuweisen', 'Alte verwenden');
    ...
    if useExisting
        ...
        if any(strcmp({vars.name}, 'cochleaCenter')) && ...
           any(strcmp({vars.name}, 'cochleaEntrance'))
            ...
            title([vLabel ': Gespeicherte Datei ohne Referenzpunkte: Mittelpunkt (1), dann Eingang (2) klicken']);
    ...
    figure;
    imshow(ImageData(1).rgb); hold on;
    title([vLabel ': Referenzpunkte: erst Mittelpunkt des Cochleamodells (1), dann Eingang (2) klicken']);
    ...
    title([vLabel ': Manual ROI creation: first draw workspace (red), then click Tip (green)']);
    ...
    title([vLabel ': Single-click to define the ROI for Tip search (green)']);
    ...
    for k = 1:numImages
        ...
        if isempty(B)
            warning(['[' vLabel '] Image ', num2str(k), ': Tip mask has no boundary, skipping']);
            continue;
        end
        B_coords = B{1};
        ...
        if isempty(CC.PixelIdxList)
            if k > 2 && all(TipCoordinates(k-1,:)) && all(TipCoordinates(k-2,:))
                warning(['[' vLabel '] Image ', num2str(k), ': No component found, using vector prediction']);
        ...
        if ~found
            warning(['[' vLabel '] Image ', num2str(k), ': prediction + progressive search failed, please mark Tip manually']);

            figure(1); clf;
            imshow(bwImg); hold on;
            title([vLabel ' | Image ', num2str(k), ': Single-click to define the ROI for Tip search']);
        ...
        if isempty(Intersection)
            warning(['[' vLabel '] Image ', num2str(k), ': No intersection found, skipping']);
            continue;
        end
        ...
        if k > 1 && ~isnan(TipCurvature(k-1))
            if abs(kappa - TipCurvature(k-1)) > curvatureThreshold
                warning(['[' vLabel '] Image ', num2str(k), ': curvature difference too large, please mark Tip manually']);

                figure(1); clf;
                imshow(bwImg); hold on;
                title([vLabel ' | Image ', num2str(k), ': Single-click to redefine Tip']);
        ...
        if CC_masked.NumObjects > 1
            warning(['[' vLabel '] Image ', num2str(k), ': masked ROI contains multiple disconnected white regions, please adjust manually']);

            figure(1); clf;
            imshow(bwImg); hold on;
            title([vLabel ' | Image ', num2str(k), ': Single-click to redefine Tip due to disconnected ROI']);
        ...
        title([vLabel ' | Image ', num2str(k), ' Tip position']);
        drawnow;
    ...
    TipCoordinates = reviewTips(ImageData, TipCoordinates, vLabel);
    ...
    %% Visualize Tip trajectory (eigene Figure pro Video, bleibt offen)
    ...
    if any(valid)
        ...
        xlabel('Frame');
        ylabel('Winkel [°]');
        title(['Winkel Referenz-/Tip-Linie (gegen Uhrzeigersinn positiv) (' vLabel ')']);
    end
...
function TipCoordinates = reviewTips(ImageData, TipCoordinates, vLabel)
...
% Rückgabe: (ggf.) korrigiertes TipCoordinates.
    ...
    hFig = figure('Name', ['Tip-Kontrolle ' vLabel], 'NumberTitle', 'off');
